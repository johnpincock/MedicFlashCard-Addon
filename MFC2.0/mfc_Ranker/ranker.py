...
# this is for my own testsing, please ignore it.
...
def answerRankButtons():
    ...
    def but(i, label):
        if i == 2:
            ...
            extra = ""
        return '''
            <td align=center><div class=spacer></div><button %s title="%s" onclick='py.link("rank%d");'>%s</button></td>''' % (extra, _("Shortcut key: %s") % i, i, label)
...
def centerLinks(self):
    links = [
        ...
        ["mfcSync", _("mfcSync"), _("Shortcut key: %s") % "M"],
    ]
    return self._linkHTML(links)


def mfcSyncLinkHandler():
    uploadToMFC()

new_link_handlers = {
            "mfcSync": mfcSyncLinkHandler,
        }
...
#Reviewer.onEnterKey = wrap(Reviewer.onEnterKey, onEnterKey, "around")
...
origSetupButtons = Reviewer._answerButtons
Reviewer._answerButtons = mySetupButtons
...
# currently checks for each media, send media list instead and check on server end
    ...
    if tokenUsed:
        ...
        syncRanks() 
        mw.progress.finish()
        showInfo("Downloaded! Restart anki to view changes!")
...
def uploadToMFC():
    mw.unloadCollection()
    mw.progress.start(label="Uploading to medicflashcards.com.... Please don't close Anki \n (This can take up to 30 seconds!)", immediate=True)
...
# send local collection
    ...
    if tokenUsed:
        ...
        tooltip("Couldn't connect, unauthorised token")
    mw.progress.finish()
...
def pragmaCheck():
    b = mw.col.db.scalar("pragma integrity_check")
    showInfo(b)
    if b == 'ok':
        pass
    else:
        showInfo("drop?")
        # mw.col.db.scalar("drop index if exists ix_cards_sched")
        # mw.col.db.scalar("CREATE INDEX ix_cards_sched on cards (did, queue, due);")
